...
public class AppResource extends AbstractResource {
    ...
    public Response getAppStatus(@QueryParam("options") String options) {
        ...
        GetSystemStatusResponse rs = ResponseBuilder.of(GetSystemStatusResponse.class)
                .build();
        List<String> lopts = Collections.EMPTY_LIST;
        if (options != null) {
            lopts = Arrays.asList(options.split(","));
        }
        rs.withAppInfo(new AppInfo()
                .withApiVersion(new SchemaProperties()
                        .getApiVersion()));
        ...
        rs.withAppConfig(new GetSystemStatusResponse.AppConfig()
                .withParams(new Param().withName("appVersion").withValue(appConfig.getVersion()))
                );
    ...
    void addSystemProperties(GetSystemStatusResponse rs) {
        ...
        for (Entry<Object, Object> p : orderedProps.entrySet()) {
            sp.getParams().add(new Param().withName(p.getKey().toString()).withValue(p.getValue().toString()));
        }
    }
    ...
    void addBuildProperties(GetSystemStatusResponse rs) {
        InputStream is = AppResource.class.getResourceAsStream("/build.properties");
        if (is == null) {
            rs.getWarnings()
                    .add(ErrorUtil.createError(ErrorUtil.FAIILED_ASSERTION,
        ...
        try {
            ...
            for (Entry<Object, Object> e : props.entrySet()) {
                appInfo.getParams().add(new Param()
                        .withName(e.getKey().toString())
                        .withValue(e.getValue().toString()));
            }
        }
        catch (IOException e) {
